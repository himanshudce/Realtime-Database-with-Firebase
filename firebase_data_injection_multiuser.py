...
def time_measurement(num):
    for i in range(4):
        time.sleep(0.5)
        response_write = int(requests.get('http://localhost:3000/firebase/read/crypto').text)
...
if __name__ == "__main__":
    ...
    p1 = multiprocessing.Process(target=load_data_firebase, args=(bitcoin_data, ))
    p2 = multiprocessing.Process(target=load_data_firebase, args=(bitcoin_data, ))
    p3 = multiprocessing.Process(target=load_data_firebase, args=(bitcoin_data, ))
